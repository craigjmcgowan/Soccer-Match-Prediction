...
```{r recent fouls form}
single_recent_model(prem_recent_fouls) %>%
  pander(split.cells = 15)
...
multi_recent_model(prem_recent_fouls) %>%
  pander(split.cells = 15)
```

# Recent Offsides
Using logistic regression, look at what timeline of recent offsides best predicts game outcomes using leave-one-season out cross validation and evaluating models on percentage of matches accurately predicted.
```{r recent offsides form}
single_recent_model(prem_recent_offsides) %>%
  pander(split.cells = 15)
...
multi_recent_model(prem_recent_offsides) %>%
  pander(split.cells = 15)
```
...
```{r recent yc form}
single_recent_model(prem_recent_ycs) %>%
  pander(split.cells = 15)
...
multi_recent_model(prem_recent_ycs) %>%
  pander(split.cells = 15)
```